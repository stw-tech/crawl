...
def crawl_patent(patent, year):
    ...
    for id in patent:
        ...
        try:
            Kind_Code = soup.find_all('td', {'align': 'RIGHT', 'width': '50%'})[1].text.replace('\n', '')
        except:
            Kind_Code = ''
        try:
        ...
        try:
            ...
            Inventors = ''
        try:
            Assignee = soup.find('td', text=re.compile('Assignee')).next_sibling.next_sibling.text
        except:
            Assignee = ''
        try:
            PCT_Filed = soup.find('td', text=re.compile('PCT Filed')).next_sibling.text.replace('\n', '')
        except:
            PCT_Filed = ''
        try:
            PCT_NO = soup.find('td', text=re.compile('PCT NO')).next_sibling.text.replace('\n', '')
        except:
            PCT_NO = ''

        link = 'http://pdfaiw.uspto.gov/.aiw?Docid={}'.format(id)
        crawled_info[id]['title'] = title
        crawled_info[id]['Kind_Code'] = Kind_Code
        crawled_info[id]['Inventors'] = Inventors
        crawled_info[id]['Assignee'] = Assignee
        ...
        crawled_info[id]['PCT_Filed'] = PCT_Filed
        crawled_info[id]['PCT_NO'] = PCT_NO
        crawled_info[id]['Publication_Date'] = Publication_Date
        crawled_info[id]['link'] = link

        tag = soup.select('center > b')
        crawled_info[id]["Abstract"], crawled_info[id]["Claims"], crawled_info[id]["Description"] = '', '', ''
        for t in tag:
            if t.text == "Abstract" :
...
def save_json(crawl_data, year):
    ...
    path = os.path.join(saving_dir, str(year), crawl_data['Publication_Number'])
...
def check_crawled(year):
    ...
    ck_path = os.path.join(saving_dir, str(year))
